...
inputsList.forEach((element)=>{
    element.insertAdjacentHTML("beforebegin", `<label class="field-label" for=${element.id}>
      ${divideWords(element.name)}
    </label>`);
    element.setAttribute("placeholder", `${divideWords(element.name[0].toUpperCase() + element.name.slice(1))}`);
});
